"use client"

import * as React from "react"
import Image from "next/image"

const companies = [
  { name: "Apple", type: "image", src: "/apple-logo-svgrepo-com.svg", width: 50 },
  { name: "Microsoft", type: "image", src: "/microsoft-logo-svgrepo-com.svg", width: 50 },
  { name: "Meta", type: "image", src: "/meta-logo-facebook-2-svgrepo-com.svg", width: 50 },
  { name: "Expo", type: "image", src: "/expo-svgrepo-com.svg", width: 50 },
  { name: "Spotify", type: "image", src: "/spotify-ar21~bgwhite.svg", width: 50 },
  { name: "Uber", type: "image", src: "/uber-svgrepo-com.svg", width: 50 },
  { name: "Figma", type: "image", src: "/figma-svgrepo-com.svg", width: 50 },
  { name: "Vercel", type: "image", src: "/vercel-svgrepo-com.svg", width: 50 },
  { name: "Netflix", type: "image", src: "/netflix-icon-svgrepo-com.svg", width: 50 },
  { name: "Stripe", type: "image", src: "/stripe-svgrepo-com.svg", width: 50 },
  { name: "Snapchat", type: "image", src: "/snapchat-logo-svgrepo-com.svg", width: 50 },
  { name: "Mailchimp", type: "image", src: "/mailchimp-svgrepo-com.svg", width: 50 },
  { name: "BMW", type: "image", src: "/bmw-logo-svgrepo-com.svg", width: 50 },
  { name: "Airbnb", type: "image", src: "/airbnb-color-svgrepo-com.svg", width: 60 },
  { name: "Shopify", type: "image", src: "/shopify-color-svgrepo-com.svg", width: 50 },
  { name: "Notion", type: "image", src: "/notion-svgrepo-com.svg", width: 50 },
  { name: "Linear", type: "image", src: "/linear-svgrepo-com.svg", width: 50 },
  { name: "GitHub", type: "image", src: "/github-svgrepo-com.svg", width: 50 },
  { name: "Discord", type: "image", src: "/discord-svgrepo-com.svg", width: 50 },
]

// Split companies into two rows
const row1 = companies.slice(0, 6)
const row2 = companies.slice(6, 12)
const row3 = companies.slice(12, 18)

export function Partners() {
  return (
    <div className="w-full py-12 mt-8 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col items-center gap-4 mb-8">
          <p className="text-xs uppercase tracking-widest text-neutral-400 font-light">
            TRUSTED BY LEADING APPLICATIONS
          </p>
        </div>
        
        {/* Marquee Container - 2 Rows */}
        <div className="space-y-8">
          {/* Row 1 */}
          <div className="relative overflow-hidden">
            <div className="flex">
              {/* First set for seamless loop */}
              <div className="flex items-center gap-16 shrink-0 animate-marquee">
                {row1.map((company, idx) => (
                  <CompanyLogo key={`row1-first-${idx}`} company={company} />
                ))}
              </div>
              {/* Duplicate set for seamless loop */}
              <div className="flex items-center gap-16 shrink-0 animate-marquee">
                {row1.map((company, idx) => (
                  <CompanyLogo key={`row1-second-${idx}`} company={company} />
                ))}
              </div>
            </div>
          </div>

          {/* Row 2 - Reverse direction */}
          <div className="relative overflow-hidden">
            <div className="flex">
              {/* First set for seamless loop */}
              <div className="flex items-center gap-16 shrink-0 animate-marquee-reverse">
                {row2.map((company, idx) => (
                  <CompanyLogo key={`row2-first-${idx}`} company={company} />
                ))}
              </div>
              {/* Duplicate set for seamless loop */}
              <div className="flex items-center gap-16 shrink-0 animate-marquee-reverse">
                {row2.map((company, idx) => (
                  <CompanyLogo key={`row2-second-${idx}`} company={company} />
                ))}
              </div>
            </div>
          </div>

          {/* Row 3 */}
          <div className="relative overflow-hidden">
            <div className="flex">
              {/* First set for seamless loop */}
              <div className="flex items-center gap-16 shrink-0 animate-marquee">
                {row3.map((company, idx) => (
                  <CompanyLogo key={`row3-first-${idx}`} company={company} />
                ))}
              </div>
              {/* Duplicate set for seamless loop */}
              <div className="flex items-center gap-16 shrink-0 animate-marquee">
                {row3.map((company, idx) => (
                  <CompanyLogo key={`row3-second-${idx}`} company={company} />
                ))}
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

function CompanyLogo({ company }: { company: typeof companies[0] }) {
  if (company.type === "svg" && company.name === "Google") {
    return (
      <div className="flex items-center justify-center opacity-40 hover:opacity-90 transition-opacity cursor-pointer shrink-0">
        <svg
          width="80"
          height="24"
          viewBox="0 0 272 92"
          fill="none"
          className="text-neutral-900 cursor-pointer"
        >
          <path d="M115.75 47.18c0 12.77-9.99 22.18-22.25 22.18s-22.25-9.41-22.25-22.18C71.25 34.32 81.24 25 93.5 25s22.25 9.32 22.25 22.18zm-9.74 0c0-7.98-5.79-13.44-12.51-13.44S80.99 39.2 80.99 47.18c0 7.9 5.79 13.44 12.51 13.44s12.51-5.55 12.51-13.44z" fill="currentColor"/>
          <path d="M163.75 47.18c0 12.77-9.99 22.18-22.25 22.18s-22.25-9.41-22.25-22.18c0-12.85 9.99-22.18 22.25-22.18s22.25 9.32 22.25 22.18zm-9.74 0c0-7.98-5.79-13.44-12.51-13.44s-12.51 5.46-12.51 13.44c0 7.9 5.79 13.44 12.51 13.44s12.51-5.55 12.51-13.44z" fill="currentColor"/>
          <path d="M209.75 26.34v39.82c0 16.38-9.66 23.07-21.08 23.07-10.75 0-17.22-7.19-19.66-13.07l8.48-3.53c1.51 3.61 5.21 7.87 11.17 7.87 7.31 0 11.84-4.51 11.84-13v-3.19h-.34c-2.18 2.69-6.38 5.04-11.68 5.04-11.09 0-21.25-9.66-21.25-22.09 0-12.52 10.16-22.26 21.25-22.26 5.29 0 9.49 2.35 11.68 4.96h.34v-3.61h9.25zm-8.56 20.92c0-7.81-5.21-13.52-11.84-13.52-6.72 0-12.35 5.71-12.35 13.52 0 7.73 5.63 13.36 12.35 13.36 6.63 0 11.84-5.63 11.84-13.36z" fill="currentColor"/>
          <path d="M225 3v65h-9.5V3h9.5z" fill="currentColor"/>
          <path d="M262.02 54.48l7.56 5.04c-2.44 3.61-8.32 9.83-18.48 9.83-12.6 0-22.01-9.74-22.01-22.18 0-13.19 9.49-22.18 20.92-22.18 11.51 0 17.14 9.16 18.98 14.11l1.01 2.52-29.65 12.28c2.27 4.45 5.8 6.72 10.75 6.72 4.96 0 8.4-2.44 10.92-6.14zm-23.27-7.98l19.82-8.23c-1.09-2.77-4.37-4.7-8.23-4.7-4.95 0-11.84 4.37-11.59 12.93z" fill="currentColor"/>
          <path d="M35.29 41.41V32H67c.31 1.64.47 3.58.47 5.68 0 7.06-1.93 15.79-8.15 22.01-6.05 6.3-13.78 9.66-24.02 9.66C16.32 69.35.36 53.89.36 34.91.36 15.93 16.32.47 35.3.47c10.5 0 17.98 4.12 23.6 9.49l-6.64 6.64c-4.03-3.78-9.49-6.72-16.97-6.72-13.86 0-24.7 11.17-24.7 25.03 0 13.86 10.84 25.03 24.7 25.03 8.99 0 14.11-3.61 17.39-6.89 2.66-2.66 4.41-6.46 5.1-11.65l-22.49.01z" fill="currentColor"/>
        </svg>
      </div>
    )
  }
  
  if (company.type === "image") {
    return (
      <div className="flex items-center justify-center opacity-40 hover:opacity-90 transition-opacity cursor-pointer shrink-0">
        <Image
          src={company.src!}
          alt={company.name}
          width={company.width}
          height={24}
          className="dark:invert"
        />
      </div>
    )
  }
  
  // Text-based logos for companies without images
  return (
    <div className="flex items-center justify-center opacity-40 hover:opacity-90 transition-opacity cursor-pointer shrink-0">
      <span className="text-neutral-900 text-sm font-light tracking-wide">
        {company.name}
      </span>
    </div>
  )
}

